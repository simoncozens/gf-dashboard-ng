...
```jsx
const updates = (await FileAttachment("./versionhistory.json").json());
var movesbydate = {}
...
display(Object.keys(movesbydate).sort().reverse().slice(0,50).map( (date) => 
<div>
    <h2>{date}</h2>
    <ul>
    {
        movesbydate[date].sort().map(([name, version, server]) => <li>
        <span class={server}>{name}</span> {version} → <span class={server}>{server}</span> </li>
        )}
    </ul>
</div>))
```
